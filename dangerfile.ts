...
if (!danger.github.pr.body || danger.github.pr.body.length < 10) {
  fail("A descrição da PR está muito curta. Por favor, adicione mais detalhes.");
}
...
sensitiveFiles.forEach(file => {
  if (danger.git.modified_files.includes(file)) {
    message(`⚠️ O arquivo **${file}** foi modificado. Certifique-se de que essa mudança foi intencional.`);
  }
});
...
if (packageJsonChanged) {
  const addedDeps = danger.git.created_files.filter(file => file.includes("package.json"));
  if (addedDeps.length) {
    message("🔧 Dependências foram adicionadas ou alteradas em `package.json`. Verifique se todas são necessárias.");
  }
}

// Verifica se o código está bem formatado (com Prettier)
const formattedFiles = danger.git.modified_files.filter(file => file.endsWith(".ts") || file.endsWith(".tsx"));
if (formattedFiles.length && !formattedFiles.every(file => file.match(/.*\.ts$/))) {
  warn("⚠️ Alguns arquivos TypeScript podem não estar corretamente formatados. Execute o Prettier.");
}

const modifiedFiles = [...danger.git.modified_files, ...danger.git.created_files];
const logFiles = modifiedFiles.filter(file => file.endsWith(".ts") || file.endsWith(".js"));
...
commitMessages.forEach(commitMessage => {
  if (!conventionalCommitRegex.test(commitMessage)) {
    warn(`⚠️ O commit "${commitMessage}" não segue o formato convencional. Utilize algo como "feat: adicionar nova funcionalidade" ou "fix: corrigir bug".`);
  }
});